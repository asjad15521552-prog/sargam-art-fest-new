...
target = """                      {adminTab === 'check_publish' && (
                        <div className="space-y-8">
                          <div className="flex justify-between items-center bg-stone-900 p-6 rounded-2xl border border-stone-800">
...
replacement = """                      {adminTab === 'check_publish' && (
                        <div className="space-y-8">
                          ...
                          <div className="text-stone-400 space-y-6">
                              ...
                              {/* Simulator Table */}
                              <div className="bg-stone-900 border border-stone-800 rounded-2xl p-6 overflow-hidden">
                                <h3 className="text-lg font-black text-amber-300 mb-6">Simulation Table</h3>
                                <div className="overflow-x-auto">
                                  <table className="w-full text-sm text-left">
                                    <thead className="text-xs uppercase bg-stone-950 text-stone-400">
                                      <tr>
                                        <th className="px-4 py-3 rounded-tl-xl">Program Code</th>
                                        <th className="px-4 py-3">Program Name</th>
                                        <th className="px-4 py-3">Team Points</th>
                                        <th className="px-4 py-3 rounded-tr-xl text-center">Action</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-stone-800">
                                      {simRows.map((row, index) => {
                                        ...
                                        return (
                                          <tr key={row.id} className="bg-stone-900/50 hover:bg-stone-900 transition-colors">
                                            <td className="px-4 py-3">
                                              <input
                                                type="text"
                                                value={row.programCode}
                                                placeholder="Type code..."
                                                className="bg-stone-950 border border-stone-800 rounded-lg p-2 text-stone-200 outline-none focus:border-amber-500/50 w-28 uppercase"
                                                onChange={(e) => {
                                            ...
                                            <td className="px-4 py-3 text-amber-100 font-bold">
                                              {selectedProg ? selectedProg.name : <span className="text-stone-600 italic">Not found</span>}
                                            </td>
                                            <td className="px-4 py-3">
                                              {selectedProg ? (
                                                <div className="flex flex-col gap-1 text-[10px]">
                                                  {Object.entries(teamPoints).map(([t, p]) => (
                                                    <span key={t} className="text-amber-400">{t}: {p}</span>
                                                  ))}
