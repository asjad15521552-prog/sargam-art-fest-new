...
target = """                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-stone-900 border border-amber-500/20 p-4 rounded-xl">
                            <div>
...
replacement = """                          {/* Top Standings */}
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                            {teamScoringList.map((team, index) => ("""

content = content.replace(target, replacement)

target2 = """                                {(dashboardViewMode === 'admin' ? adminCategoryRankData : categoryRankData).map(catData => ("""
replacement2 = """                                {categoryRankData.map(catData => ("""
content = content.replace(target2, replacement2)

target3 = """                                {(dashboardViewMode === 'admin' ? adminTopPerformersList : globalTopStudents).map((student, idx) => ("""
replacement3 = """                                {globalTopStudents.map((student, idx) => ("""
content = content.replace(target3, replacement3)

target4 = """                                {(dashboardViewMode === 'admin' ? adminTopPerformersList : globalTopStudents).length === 0 && ("""
replacement4 = """                                {globalTopStudents.length === 0 && ("""
content = content.replace(target4, replacement4)

with open('src/App.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Dashboard revert patched")
