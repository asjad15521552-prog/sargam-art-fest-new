...
pattern = re.compile(
    r'<div className="flex items-center gap-2">\s*<button[^>]+onClick=\{\(\) => \{\s*const updatedPrograms = programs\.map\(p => p\.id === prog\.id \? \{ \.\.\.p, isDashboardPublished: !p\.isDashboardPublished \} : p\);.*?<Trash2 className="w-4 h-4" />\s*</button>\s*</div>',
    re.DOTALL
)
...
replacement = """<div className="flex flex-col items-end gap-1.5">
                                              ...
                                              <div className="flex flex-col xl:flex-row items-end xl:items-center gap-1.5 mt-1">
                                                <button 
                                                  onClick={() => {
...
if new_content != content:
    with open('src/App.tsx', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully updated toggles!")
else:
